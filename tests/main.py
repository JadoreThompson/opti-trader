...
from .intergration.test_api import test_order_close, test_order_creation, test_order_modification


async def run_order_creation_test() -> None:
    await asyncio.gather(
        *[
            test_order_creation(10_000, 0.1, market_type=MarketType.FUTURES)
            for _ in range(1)
        ]
    )
...
if __name__ == "__main__":
    asyncio.run(run_order_close_test())
